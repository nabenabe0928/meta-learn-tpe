...
def get_config_space(search_space: Dict[str, ParameterSettings], hp_module_path: str) -> CS.ConfigurationSpace:
    ...
    for param_name, settings in search_space.items():
        ...
        if settings["param_type"] not in type_choices:
            raise ValueError(
                "`param_type` in params.json must have one of {}, "
                "but got {}.".format(type_choices, settings["param_type"])
            )

        if len(set(settings.keys()) - settings_keys) >= 1:
            raise ValueError("params.json must not include the keys not specified in {}.".format(settings_keys))

        if "log" in settings.keys():
            if settings["log"] not in bool_json.__args__:  # type: ignore
                raise ValueError(
                    '`log` in params.json must have either "True" or "False", ' "but got {}.".format(settings["log"])
                )
        ...
        cs.add_hyperparameter(
            get_hyperparameter(
                param_name=param_name, config_type=config_type, settings=settings, hp_module_path=hp_module_path
            )
        )
...
def check_value_range(hp_name: str, config: CSH.Hyperparameter, val: NumericType) -> None:
    if val < config.lower or val > config.upper:
        raise ValueError(
            "The sampled value for {} must be [{},{}], but got {}.".format(hp_name, config.lower, config.upper, val)
        )
...
def store_results(
    best_config: Dict[str, np.ndarray], logger: Logger, observations: Dict[str, np.ndarray], file_name: str
) -> None:
